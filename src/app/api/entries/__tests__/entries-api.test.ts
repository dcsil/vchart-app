...
// Updated: Mock the Entry model to reflect the new structure with a `vitalSigns` object
...
describe("Entries API Endpoints", () => {
  ...
  describe("POST /api/entries", () => {
    ...
    it("should return 400 if patient ID is missing", async () => {
      // Mock request with auth cookie but missing patientId (vitalSigns provided though)
      const request = {
        cookies: {
          get: jest.fn().mockReturnValue({ value: "testuser" }),
        },
    ...
    it("should return 404 if patient is not found", async () => {
      // Mock request with auth cookie and invalid patientId
      const request = {
        cookies: {
          get: jest.fn().mockReturnValue({ value: "testuser" }),
        },
    ...
    it("should handle errors when creating entry", async () => {
      ...
      expect(response.status).toBe(500);
      const responseData = await response.json();
      expect(responseData.message).toBe("Failed to add entry: Database error");

      expect(log).toHaveBeenCalledWith(
        expect.stringContaining("Error adding entry"),
        "error"
      );
    });
  });
  ...
  describe("PUT /api/entries", () => {
    ...
    it("should update an entry successfully", async () => {
      ...
      expect(mockEntry.vitalSigns.temperature).toBe("99.1");
    ...
    it("should handle errors when updating entry", async () => {
      ...
      expect(log).toHaveBeenCalledWith(
        expect.stringContaining("Error updating entry"),
        "error"
      );
    });
  });
